...
resource "aws_nat_gateway" "nat_gateway_az1" {
  allocation_id = aws_eip.eip_for_nat_gateway_az1.id #allocate elaastic ip to nat gateway
  ...
  depends_on = [var.internet_gateway] #according to the terraform documentation, a nat gateway must depend on the internet gateway
}

# create nat gateway in public subnet az2
resource "aws_nat_gateway" "nat_gateway_az2" {
  allocation_id = aws_eip.eip_for_nat_gateway_az2.id #allocate elastic ip to nat gateway
...
# create private route table az1 and add route through nat gateway az1. We will use this privaate route taable to route traffic through
resource "aws_route_table" "private_route_table_az1" {
  vpc_id            = var.vpc_id

  route {
    cidr_block      = "0.0.0.0/0" #routes traffic anywhere
